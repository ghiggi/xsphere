...
ds1 = ds.sphere.add_nodes_from_pygsp(
    pygsp_graph=pg.graphs.SphereHealpix(subdivisions=16, k=20, nest=True)
)
...
for ax in fg.axes.flat:
    ...
    ax.add_feature(
        cartopy.feature.NaturalEarthFeature(
            "physical", "land", "110m", facecolor="black", alpha=0.4
        )
    )

# - Superimpose contours to each panel
fg.map_dataarray_unstructured(
    fg,
    xsphere.plot.contour,
    subplot_kws={"projection": crs_proj},
    contour_labels_format="{:.0f}".format,
...
#### Plot nodes
fig, ax = plt.subplots(1, 1, subplot_kw=dict(projection=crs_proj))
p = ds.sphere.plot_nodes(ax=ax, transform=crs_ref, add_background=True, c="orange")
plt.show()
